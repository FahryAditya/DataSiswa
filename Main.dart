...
import 'dart:convert';
import 'dart:math'; // Untuk animasi putaran (Transform.rotate)

// --- 1. MODEL DATA & KONSTANTA (SAMA SEPERTI SEBELUMNYA) ---
...
// Tambahkan TickerProviderStateMixin untuk menggunakan AnimationController
class _DataParserScreenState extends State<DataParserScreen> with TickerProviderStateMixin {
  ...
  // ANIMATION: Controller untuk Staggered Entrance (Membuat UI muncul bertahap)
  ...
  void initState() {
    super.initState();
    // Inisialisasi Controller Spin
    _processButtonController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 700),
    );

    // Inisialisasi Controller Entrance
    _entranceController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 800),
    );
    // Animasi Geser dari bawah
    _slideAnimation = Tween<Offset>(
      begin: const Offset(0, 0.3), // Mulai sedikit di bawah
      end: Offset.zero,
    ).animate(CurvedAnimation(
      parent: _entranceController,
      curve: Curves.easeOutCubic, // Curve yang halus
    ));
    // Animasi Fade In
    _fadeAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(_entranceController);

    _loadData(); // Muat data
    // Jalankan animasi entrance setelah loading data selesai
  ...
  // --- 4. LOGIKA BISNIS & FILE HANDLING (SAMA DENGAN TAMBAHAN ANIMASI) ---
  ...
  /// Memproses Input Otomatis (Dengan Animasi Spin)
  void _processAutomaticInput() async {
    ...
    // 1. Mulai Animasi Spin
    setState(() { _isProcessing = true; });
    _processButtonController.repeat(); // Loop animasi

    // Simulasi waktu proses
    await Future.delayed(const Duration(milliseconds: 1500));

    // 2. Logika Parsing
    final regex = RegExp(r'([\w\s]+)\s+kelas\s+(10|11|12)\s+([\w]+)', caseSensitive: false);
  ...
  void _addManualInput() {
    ...
    // Tambahkan efek visual (seperti shake ringan pada tombol)
  }

  void _clearAllData() async {
    // ... (Logika Clear Data)
    // Logika sudah memadai, tidak perlu animasi tambahan di sini
  ...
  Widget build(BuildContext context) {
    ...
    // Menggunakan SlideTransition dan FadeTransition untuk Entrance Animasi
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 16),
                _buildSearchInput(),
                _buildHierarchyTable(groupedData),
                const SizedBox(height: 16),
                _buildStatisticsLabel(),
              ],
            ),
          ),
          // FAB untuk Proses Otomatis (Opsional, mengganti tombol di Card)
  ...
  // Widget: FAB (Mengganti tombol 'Proses Otomatis' di Card)
  ...
  // Widget: Card Input Otomatis (Diubah untuk hanya menampung input)
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Tombol dihilangkan, diganti FAB
          ],
        ),
      ),
    );
  ...
  // Widget: Card Input Manual
  Widget _buildManualInputCard() {
    // ... (DropdownButtonFormField dll, sama)
  ...
  // Widget Pembantu untuk Tombol (untuk konsistensi dan kemudahan perubahan shape)
  ...
  // Widget: Tabel Hierarki (ExpansionTile Bertingkat dengan Animasi)
      ...
      child: ClipRRect(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            // Tingkat 1: Sekolah (ExpansionTile dengan AnimatedContainer)
            return AnimatedContainer(
              ...
              child: ExpansionTile(
                ...
                // Gunakan AnimatedSwitcher untuk animasi konten saat ekspansi
                children: [
                  AnimatedSwitcher(
                    ...
                    child: Column(
                      ...
                      children: jurusanData.entries.map((entryJurusan) {
                        ...
                        // Tingkat 2: Jurusan (ExpansionTile)
                        return Padding(
